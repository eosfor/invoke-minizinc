...
# Generated at: 10/27/2023 22:46:05
...
<#
.SYNOPSIS
Could not find /Applications/MiniZincIDE.app/Contents/Resources/minizinc to generate help.
...
PROCESS {
    ...
    if ($__boundParameters["Debug"]){wait-debugger}
    if ( $__boundParameters["Verbose"]) {
         Write-Verbose -Verbose -Message "C:\Program Files\MiniZinc\minizinc.exe"
         $__commandArgs | Write-Verbose -Verbose
    }
    $__handlerInfo = $__outputHandlers[$PSCmdlet.ParameterSetName]
    if (! $__handlerInfo ) {
    ...
    $__handler = $__handlerInfo.Handler
    if ( $PSCmdlet.ShouldProcess("C:\Program Files\MiniZinc\minizinc.exe $__commandArgs")) {
    # check for the application and throw if it cannot be found
        if ( -not (Get-Command -ErrorAction Ignore "C:\Program Files\MiniZinc\minizinc.exe")) {
          throw "Cannot find executable 'C:\Program Files\MiniZinc\minizinc.exe'"
        }
        if ( $__handlerInfo.StreamOutput ) {
            if ( $null -eq $__handler ) {
                & "C:\Program Files\MiniZinc\minizinc.exe" $__commandArgs
            }
            else {
                & "C:\Program Files\MiniZinc\minizinc.exe" $__commandArgs 2>&1| Push-CrescendoNativeError | & $__handler
            }
        }
        else {
            $result = & "C:\Program Files\MiniZinc\minizinc.exe" $__commandArgs 2>&1| Push-CrescendoNativeError
            & $__handler $result
        }
    }
    # be sure to let the user know if there are any errors
...
minizinc.exe: Unrecognized option or bad format `-?'
More info with "C:\Program Files\MiniZinc\minizinc.exe --help"

.DESCRIPTION See help for C:\Program Files\MiniZinc\minizinc.exe
...
PROCESS {
    ...
    if ($__boundParameters["Debug"]){wait-debugger}
    if ( $__boundParameters["Verbose"]) {
         Write-Verbose -Verbose -Message "/usr/local/bin/minizinc"
         $__commandArgs | Write-Verbose -Verbose
    }
    $__handlerInfo = $__outputHandlers[$PSCmdlet.ParameterSetName]
    if (! $__handlerInfo ) {
    ...
    $__handler = $__handlerInfo.Handler
    if ( $PSCmdlet.ShouldProcess("/usr/local/bin/minizinc $__commandArgs")) {
    # check for the application and throw if it cannot be found
        if ( -not (Get-Command -ErrorAction Ignore "/usr/local/bin/minizinc")) {
          throw "Cannot find executable '/usr/local/bin/minizinc'"
        }
        if ( $__handlerInfo.StreamOutput ) {
            if ( $null -eq $__handler ) {
                & "/usr/local/bin/minizinc" $__commandArgs
            }
            else {
                & "/usr/local/bin/minizinc" $__commandArgs 2>&1| Push-CrescendoNativeError | & $__handler
            }
        }
        else {
            $result = & "/usr/local/bin/minizinc" $__commandArgs 2>&1| Push-CrescendoNativeError
            & $__handler $result
        }
    }
    # be sure to let the user know if there are any errors
...
<#
.SYNOPSIS
Could not find /usr/local/bin/minizinc to generate help.

.DESCRIPTION See help for /usr/local/bin/minizinc
